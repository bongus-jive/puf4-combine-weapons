...
function MeleeCombo:init()
	togglecd = 0
	animator.setAnimationState("blade", "off")
...
function MeleeCombo:update(dt, fireMode, shiftHeld)
  WeaponAbility.update(self, dt, fireMode, shiftHeld)
	...
	activeItem.setCursor("/combine/stunstick/cursor/"..animator.animationState("blade")..".cursor")
  ...
  if not self.weapon.currentAbility
	and self:shouldActivate()
	and togglecd == 0 
	and animator.animationState("blade") == "on" then
    self:setState(self.windup)
  end

  if not self.weapon.currentAbility
	and (self.fireMode == "alt" or (self.fireMode == "primary" and animator.animationState("blade") == "off"))
	and togglecd == 0 then
    if animator.animationState("blade") == "off" then
			animator.setAnimationState("blade", "on")
		elseif animator.animationState("blade") == "on" then
			animator.setAnimationState("blade", "off")
		end
		togglecd = 25
		animator.playSound("toggle")
		animator.burstParticleEmitter("toggle")
  end
end
